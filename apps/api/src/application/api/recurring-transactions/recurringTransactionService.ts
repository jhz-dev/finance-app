...
export const createRecurringTransaction = async (
	data: Omit<RecurringTransaction, "id" | "budgetId" | "userId" | "createdAt" | "updatedAt">,
	budgetId: string,
	userId: string,
) => {
	...
	if (member && member.role !== "ADMIN" && member.role !== "EDITOR") {
		throw new Error(
			"You do not have permission to create a recurring transaction in this budget",
		);
	}
...
export const updateRecurringTransaction = async (
	id: string,
	data: Partial<Omit<RecurringTransaction, "id" | "budgetId" | "userId" | "createdAt" | "updatedAt">>,
	userId: string,
) => {
	...
	if (
		!member &&
		(await prisma.budget.findFirst({
			where: { id: transactionToUpdate.budgetId, ownerId: userId },
		})) === null
	) {
		throw new Error("You are not a member of this budget");
	}
...
export const deleteRecurringTransaction = async (
	...
	if (
		!member &&
		(await prisma.budget.findFirst({
			where: { id: transactionToDelete.budgetId, ownerId: userId },
		})) === null
	) {
		throw new Error("You are not a member of this budget");
	}
