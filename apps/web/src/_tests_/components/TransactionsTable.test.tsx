...
describe("TransactionsTable", () => {
  ...
  const mockUseDeleteTransaction = (
    options: Partial<ReturnType<typeof useDeleteTransaction>>
  ) => {
  ...
  it("should render a list of transactions", async () => {
    mockUseDeleteTransaction({});
    await render({
      component: () => (
        <TransactionsTable transactions={transactions} budgetId={budgetId} />
      ),
    });
  ...
  it("should render an empty state message", async () => {
    ...
    await render({
      component: () => (
        <TransactionsTable transactions={[]} budgetId={budgetId} />
      ),
    });
  ...
  it("should display a success message", async () => {
    mockUseDeleteTransaction({ isSuccess: true });
    await render({
      component: () => (
        <TransactionsTable transactions={[]} budgetId={budgetId} />
      ),
  ...
  it("should display an error message", async () => {
    mockUseDeleteTransaction({ isError: true });
    await render({
      component: () => (
        <TransactionsTable transactions={[]} budgetId={budgetId} />
      ),
