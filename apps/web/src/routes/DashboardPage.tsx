...
function DashboardPage() {
  ...
  const { isPending, isError, isSuccess, data, error } = useQuery<PaginatedBudgets, Error>({
  ...
  return (
    <div className="mt-4">
      ...
      {isError && (
        <p className="text-red-500">{t('Error fetching budgets: {{message}}', { message: error.message })}</p>
      )}
      {isSuccess &&
        (data.budgets.length > 0 ? (
          <>
            ...
            <div className="flex justify-center items-center mt-6 space-x-4">
              ...
              <span className="text-white">{t('Page')} {page}</span>
              <Button
          ...
          <div className="text-center mt-20">
            <h2 className="text-2xl font-semibold text-white">{t('No Budgets Yet')}</h2>
